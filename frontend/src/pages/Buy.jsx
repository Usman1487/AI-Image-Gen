...
const Buy = () => {
  ...
  return (
    ...
    >
      {/* Change 1: Badge styling (Dark background friendly) */}
      ...
      <div className='flex flex-wrap justify-center gap-6 text-left'>
        {plans.map((item, idx)=>(
            <div 
              key={idx} 
              // Change 3: Card Styling (Glassmorphism)
              // - bg-white hata kar bg-white/10 (Dark Glass)
              // - text-black hata kar text-white
              // - border-slate-700
              className='bg-white/10 drop-shadow-lg border border-slate-700 rounded-xl py-12 px-8 text-white hover:scale-105 transition-all duration-500 hover:border-cyan-500/50'
            > 
                <img src={logo} alt="" className='w-20 invert opacity-80' /> {/* Optional: Logo ko white karne ke liye 'invert' lagaya hai agar logo black hai to */}
                
                <p className='pt-3 pb-1 font-semibold text-lg mt-3'>{item.id}</p>
                
                {/* Typo Fix: 'txet-sm' ko 'text-sm' kiya aur color light gray kiya */}
                ...
                {/* Change 4: Purchase Button
                   - bg-gray-900 (Black) dikhta nahi dark theme mein.
                   - Isay 'bg-white' text-black banaya (High Contrast).
                */}
